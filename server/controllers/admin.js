...
exports.orders = async (req, res) => {
    try {
        const { sort, order, page, perPage, startDate, endDate } = req.body;
...
//TOP CARD: total order - total user - total product - total income this month
//LIST CARD: top selling product - top order user
//CHART:
// COLUMN: Income each month
// PIE: CATEGORY
exports.getDashboard = async (req, res) => {
    ...
    const topOrderUsers = topOrderUsersRaw.map((e, i) => ({...e, totalOrders: topUsersWithMostOrders[i].totalOrders}))
    ...
    const monthlyTotalAmounts = incomeByMonth.map(item => ({month: item._id.month,totalAmount: item.totalAmount}));
